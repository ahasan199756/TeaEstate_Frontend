...
const Analytics = () => {
  ...
  useEffect(() => {
    // Sync with your LocalStorage keys
  ...
  // Logic: Only sum base price of items actually in stock or total catalog value
  const totalStockValue = realData.products.reduce((sum, p) => {
  ...
  const glowGreen = "shadow-[0_0_20px_rgba(16,185,129,0.1)]";

  // DYNAMIC INSIGHT LOGIC
  const getAiInsight = () => {
    if (realData.orders.length === 0) return "System awaiting initial transaction flow to calibrate market positioning.";
    const lowStockItems = realData.products.filter(p => Number(p.stock) < 10).length;
    if (lowStockItems > 0) return `Critical: ${lowStockItems} assets are showing stock depletion. Immediate replenishment advised.`;
    return "Revenue velocity is stable. Neural patterns suggest high retention in existing categories.";
  };

  return (
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-700 p-6 max-w-[1600px] mx-auto text-slate-200">
        ...
        <div className="bg-white/5 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 flex items-center gap-6">
          <div className="text-right">
            ...
            <p className="text-xl font-mono font-bold text-white uppercase">
              {realData.products.length > 0 ? 'Optimal' : 'Standby'}
            </p>
          </div>
      ...
      <div className={`${obsidianCard} p-1 ${glowGreen}`}>
        <div className="bg-gradient-to-br from-emerald-500/10 to-transparent p-8 md:p-10 rounded-[2.4rem] flex flex-col lg:flex-row gap-12 items-center">
          <div className="relative group">
            <div className="absolute inset-0 bg-emerald-500/30 blur-3xl group-hover:bg-emerald-500/50 transition-all"></div>
            <div className="relative bg-emerald-500 p-6 rounded-[2rem] text-black shadow-2xl shadow-emerald-500/20">
          ...
          <div className="flex-1 space-y-4">
            ...
            <p className="text-2xl md:text-3xl font-bold text-white leading-[1.2] tracking-tight">
              Total inventory value is <span className="text-emerald-500">৳{totalStockValue.toLocaleString()}</span>. 
              <br className="hidden md:block" />
              <span className="text-white/80">{getAiInsight()}</span>
            </p>
          </div>

          <button className="whitespace-nowrap px-10 py-5 bg-white text-black font-black uppercase text-[11px] tracking-[0.2em] rounded-full hover:bg-emerald-500 hover:scale-105 transition-all shadow-xl shadow-white/5">
            Optimize Catalog
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className={`${obsidianCard} p-10 group hover:border-emerald-500/40 transition-all`}>
          ...
          <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Total Revenue</p>
          <h3 className="text-6xl font-black text-white font-mono tracking-tighter">৳{totalRevenue.toLocaleString()}</h3>
          <div className="mt-8 h-1 w-full bg-white/5 rounded-full overflow-hidden">
             <div className="h-full bg-emerald-500 shadow-[0_0_10px_#10b981]" style={{ width: '65%' }}></div>
          </div>
        </div>
        ...
        <div className={`${obsidianCard} p-10`}>
          ...
          <p className="text-[10px] font-bold text-emerald-500 mt-6 flex items-center gap-2 italic uppercase tracking-widest">
          ...
          <table className="w-full text-left">
            <thead className="bg-black text-[10px] font-black uppercase text-white/30 tracking-[0.4em]">
              <tr>
                ...
                <th className="p-8 text-right">Integrity Score</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5 text-sm text-white font-bold">
              {realData.categories.map((cat, i) => {
                ...
                // Simple integrity: Categories with no products have lower integrity
                const integrity = count > 0 ? (90 + Math.random() * 9).toFixed(1) : (0.0).toFixed(1);
                ...
                return (
                  <tr key={i} className="hover:bg-emerald-500/[0.03] transition-colors group">
                    ...
                    <td className="p-8">
                       <div className={`flex items-center gap-2 text-[10px] px-4 py-1.5 rounded-full border ${count > 0 ? 'border-emerald-500/30 text-emerald-500' : 'border-red-500/30 text-red-500'}`}>
                          {count > 0 ? <ShieldCheck size={12}/> : <AlertCircle size={12}/>}
                          {count > 0 ? 'NOMINAL' : 'DEPLETED'}
                       </div>
                    </td>
