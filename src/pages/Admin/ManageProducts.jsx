...
import { Edit3, Trash2, Plus, Search, ChevronLeft, ChevronRight, Package, AlertTriangle } from 'lucide-react'; // Added icons
...
const ManageProducts = () => {
  ...
  // HELPER: Get stock status style
  const getStockStatus = (count) => {
    const stock = Number(count);
    if (stock <= 0) return { label: 'Out of Stock', color: 'bg-red-500/20 text-red-500 border-red-500/20' };
    if (stock < 10) return { label: 'Low Stock', color: 'bg-orange-500/20 text-orange-500 border-orange-500/20' };
    return { label: 'In Stock', color: 'bg-emerald-500/20 text-emerald-500 border-emerald-500/20' };
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/5 bg-white/[0.02]">
              ...
              <th className="p-6 text-[10px] font-black uppercase text-emerald-500 tracking-widest">Inventory</th> {/* NEW HEADER */}
                  ...
                  <td className="p-6">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center gap-2">
                        <Package size={14} className={item.stock < 10 ? 'text-orange-500' : 'text-emerald-500'} />
                        <span className="text-white font-mono font-bold text-sm">{item.stock || 0}</span>
                        <span className="text-white/20 text-[9px] font-black uppercase tracking-tighter">Units</span>
                      </div>
                      <div className={`inline-flex items-center px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${status.color} w-fit`}>
                        {item.stock < 10 && <AlertTriangle size={8} className="mr-1" />}
                        {status.label}
                      </div>
                    </div>
                  </td>
