...
/**
 * RevealText (kept from your code)
 */
...
/**
 * GTA6-ish scroll reactive text band (Hero -> Featured)
 * - Moves big text left/right based on scroll
 * - Adds blur + fade + scale while entering
 * - NO duplicate section needed
 */
const ScrollTextBand = ({
  topText = "ESTATE GRADE",
  mainText = "PREMIUM TEA.",
  subText = "MODERN ESTATE COMMERCE.",
  desc = "Limited harvests, direct sourcing, and a checkout flow designed to feel premium.",
}) => {
  const ref = useRef(null);
  ...
  const xMain = useTransform(scrollYProgress, [0, 1], ["-22%", "22%"]);
  const xSub = useTransform(scrollYProgress, [0, 1], ["22%", "-22%"]);

  // cinematic: fade + scale + blur
  const opacity = useTransform(scrollYProgress, [0.05, 0.2, 0.9], [0, 1, 1]);
  const blur = useTransform(scrollYProgress, [0.05, 0.2], ["14px", "0px"]);
  const scale = useTransform(scrollYProgress, [0.1, 0.35], [0.98, 1]);

  return (
    <section
      ref={ref}
      className="relative py-20 md:py-28 overflow-hidden bg-[#062016]"
    >
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[1000px] h-[1000px] bg-emerald-500/10 blur-[140px] rounded-full" />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/10 to-[#062016]" />
      </div>
      ...
      >
        <p className="text-emerald-400 font-black tracking-[0.5em] uppercase text-[10px] mb-6">
          {topText}
        </p>

        <motion.h2
          style={{ x: xMain }}
          className="text-[44px] md:text-[92px] font-black uppercase tracking-tighter leading-[0.85]"
        >
          {mainText}
        </motion.h2>
        ...
        <motion.h2
          style={{ x: xSub }}
          className="text-[44px] md:text-[92px] font-black uppercase tracking-tighter leading-[0.85] text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 via-white to-emerald-200 mt-1"
        >
          {subText}
        </motion.h2>

        <p className="mt-7 text-white/45 text-base md:text-xl leading-relaxed max-w-2xl">
          {desc}
        </p>

        <div className="mt-10 flex items-center gap-3 text-white/30 uppercase font-black tracking-[0.35em] text-[9px]">
          <span>Scroll</span>
          <span className="inline-block w-10 h-[1px] bg-white/20" />
          <motion.span
            initial={{ x: 0 }}
            animate={{ x: 10 }}
            transition={{
...
const Home = () => {
  ...
  // 2) Measure carousel width (triplicated list)
  useEffect(() => {
  ...
  const carouselList = useMemo(() => {
    ...
    return [...featuredItems, ...featuredItems, ...featuredItems];
  }, [featuredItems]);
  ...
  return (
    <div className="relative min-h-screen selection:bg-emerald-500 selection:text-white bg-[#062016] text-white antialiased">
        ...
        {notification.show && (
          ...
          >
            <div className="bg-white/10 backdrop-blur-2xl px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-white/20">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-emerald-400 leading-none mb-1">
                  Added to Cart
                </p>
      ...
      {/* SCROLL REACTIVE BAND (Hero -> Featured) */}
      <ScrollTextBand
        topText="CRAFTED LIKE LUXURY"
        mainText="PREMIUM TEA."
        subText="MODERN ESTATE COMMERCE."
        desc="Limited harvests, transparent sourcing, and a checkout experience built to feel refined."
      />
        ...
        <div className="relative z-10 container mx-auto px-6 lg:px-16 mb-12">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6">
            <div className="space-y-2">
              <RevealText className="text-emerald-400 font-black tracking-[0.45em] uppercase text-[10px]">
                Estate Selection
              </RevealText>
            ...
            <Link
              to="/products"
              className="group flex items-center gap-2 text-white/60 text-xs font-bold tracking-widest hover:text-emerald-400 transition-colors duration-300"
            >
              <span className="border-b border-emerald-500/20 group-hover:border-emerald-400 pb-1 uppercase">
        ...
        <div className="relative border-y border-white/5 py-10 group">
          {featuredItems.length > 0 ? (
            ...
            <div className="text-center py-20">
              <p className="text-white/20 font-black text-sm uppercase tracking-[0.4em] italic">
                No varieties currently featured
              </p>
            </div>
          )}
        </div>
      ...
      <section className="py-24 md:py-40 bg-[#04140e] px-6 lg:px-16 overflow-hidden">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            <RevealText className="text-emerald-400 font-bold tracking-[0.5em] uppercase text-xs">
              The Origin
            </RevealText>
            <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.85]">
              <RevealText>Sourced at the</RevealText>
              <RevealText className="text-emerald-200/30 italic font-serif lowercase">
                edge of the world.
              </RevealText>
            </h2>
            <p className="text-xl text-white/50 max-w-md leading-relaxed">
              Every leaf tells a story of the high-altitude soil and the misty
              mornings of our estate. We believe in transparency from crop to
              cup.
            </p>
            <motion.button
              whileHover={{ y: -5 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
            <div className="space-y-12 lg:sticky lg:top-40">
              ...
              <div className="space-y-6">
                <p className="text-white/30 uppercase font-black tracking-[0.3em] text-[10px]">
                  Based in Sylhet — Exporting Worldwide
                </p>
                <div className="flex flex-col gap-4">
                  <a
                    href="mailto:hello@teaEstate.com"
                    className="text-3xl md:text-5xl font-bold hover:text-emerald-400 transition-colors duration-300"
                  >
                    hello@teaestate.com
                  </a>
                  <a
            ...
            <form className="flex flex-col gap-10 bg-white/[0.02] backdrop-blur-3xl border border-white/10 p-8 md:p-16 rounded-[60px] shadow-2xl">
              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black tracking-widest text-emerald-500/60 uppercase ml-1">
                    Your Name
                  </label>
                  <input
                    type="text"
                    placeholder="EX. JOHN DOE"
                    className="w-full border-b border-white/10 py-4 bg-transparent font-bold text-white placeholder:text-white/10 focus:outline-none focus:border-emerald-500 transition-colors"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black tracking-widest text-emerald-500/60 uppercase ml-1">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows="4"
                    className="w-full border-b border-white/10 py-4 bg-transparent font-bold text-white placeholder:text-white/10 focus:outline-none focus:border-emerald-500 transition-colors resize-none"
                  ></textarea>
                </div>
              </div>
      ...
      <footer className="py-12 border-t border-white/5 bg-black text-center">
        <p className="text-white/20 text-[10px] font-bold tracking-[0.5em] uppercase">
          © 2026 Tea Estate Premium. All Rights Reserved.
        </p>
      </footer>
    </div>
  );
