...
import { useCart } from "../Cart/useCart"; // ✅ FIXED import
    ...
    <>
      ...
      >
        <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
          <Link
            to="/"
            className="text-3xl font-extrabold tracking-tighter text-white uppercase"
          >
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <button
                className={`text-white hover:text-green-400 transition-colors flex items-center gap-2 pb-2 ${
                  user ? "text-green-400" : ""
                }`}
              >
                <IoPersonOutline className="text-2xl" />
                {user && (
                  <span className="text-[10px] font-bold uppercase tracking-tighter hidden lg:block">
              ...
              >
                {!user ? (
                  <>
                    <Link
                      to="/login"
                      className="block px-6 py-2 text-sm font-bold text-green-950 hover:bg-green-50 transition-colors"
                    >
                      LOG IN
                    </Link>
                    <Link
                      to="/register"
                      className="block px-6 py-2 text-sm font-medium text-gray-500 hover:bg-green-50 transition-colors"
                    >
                      CREATE ACCOUNT
                    </Link>
                  </>
                  ...
                  <>
                    ...
                    <Link
                      to="/profile"
                      className="flex items-center gap-3 px-6 py-2 text-sm font-medium text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors"
                    >
                    ...
                    <Link
                      to="/profile/orders"
                      className="flex items-center justify-between px-6 py-2 text-sm font-medium text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors"
            ...
            <Link
              to="/products"
              className="hidden md:block bg-green-600 hover:bg-green-500 text-white px-7 py-2.5 rounded-full font-bold text-xs tracking-widest transition-all transform hover:scale-105 shadow-lg"
            >
              SHOP NOW
            </Link>
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed inset-0 h-screen w-full bg-green-950 transition-all duration-700 z-60 flex flex-col items-center justify-center ${
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <Link
                  to={link.path}
                  onClick={() => setIsOpen(false)}
                  className="text-4xl font-black text-white hover:text-green-400 transition-colors uppercase"
                >
                  {link.name}
                </Link>
              </li>
